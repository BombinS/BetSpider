    ...
    def process(self):
        try:
            ...
            for i in range(rows.count()):
                ...
                self.matchInfo["oddHomeWin"].append(cells.nth(3).inner_text())
                self.matchInfo["oddDraw"].append(cells.nth(4).inner_text())
                self.matchInfo["oddAwayWin"].append(cells.nth(5).inner_text())
            ...
            logging.info(f"Успешное распознавание средних коэффициентов {self.matchInfo["oddHomeWin"]} {self.matchInfo["oddDraw"]} {self.matchInfo["oddAwayWin"]} из {rows.count()} вариантов")
        except Exception as e:
            logging.error(f"Ошибка распознавания коэффициентов 1X2 - {e}")
